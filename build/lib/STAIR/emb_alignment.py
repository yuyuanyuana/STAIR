...
class Emb_Align(object):
    ...
    def train_hgat(self,
                   ...
                   mini_batch: bool = False,   # ✅ 新增参数
                   batch_size: int = 1024,     # ✅ mini-batch 时的批量大小
                   num_neighbors: list = [10, 5], # ✅ 每层采样邻居数
                   plot: bool = False):
        '''
        Training step HGAT module.

        Parameters
        ----------
        gamma : float
            Weight of homogeneous representation in SE. 
        epoch_hgat : int
            Total training epochs.
        mini_batch : bool
            是否使用小批量采样 (NeighborLoader)，默认 False 即整图训练。
        batch_size : int
            mini-batch 采样时的批大小。
        num_neighbors : list
            mini-batch 采样时每层的邻居数。
        '''
        if self.make_log:
            ...
            self.makeLog(f"  Mini-batch training: {mini_batch}")
        ...
        # ✅ 如果 mini_batch 为 True，构建 NeighborLoader
        if mini_batch:
            train_loader = NeighborLoader(
                self.data_hgat,
                num_neighbors={key: num_neighbors for key in self.data_hgat.edge_types},
                batch_size=batch_size,
                input_nodes=None,  # 默认采样所有类型的节点
                shuffle=True
            )
        else:
            # 整图训练
        ...
        for epoch in tqdm(range(epoch_hgat)):
            ...
            if mini_batch:
                ...
                for batch in train_loader:
                    ...
                    out = self.hgat(batch.x_dict, batch.edge_index_dict, 
                                    return_semantic_attention_weights=False)

                    re_loss_batch = 0
                    si_loss_batch = 0
                    for node_type, x in out.items():
                        re_loss_batch += F.mse_loss(x, batch[node_type].x)
                        if si_weight != 0 and node_type in self.kernals:
                            si_loss_batch += F.mse_loss(self.kernals[node_type].to(self.device), 
                                                        torch.mm(x, x.T))

                    if si_weight != 0:
                        loss = re_weight * re_loss_batch + si_weight * si_loss_batch
                    else:
                        loss = re_weight * re_loss_batch

                    loss.backward()
                    optimizer_hgat.step()
                    optimizer_hgat.zero_grad()

                    re_loss += re_loss_batch.item()
                    si_loss += si_loss_batch.item() if si_weight != 0 else 0
            else:
                # ✅ 整图训练
                out = self.hgat(self.data_hgat.x_dict, self.data_hgat.edge_index_dict, 
                                return_semantic_attention_weights=False)
            ...
            if mini_batch:
                if si_weight != 0:
                    loss_list.append([re_loss + si_loss, re_loss, si_loss])
                else:
                    loss_list.append([re_loss, re_loss])
